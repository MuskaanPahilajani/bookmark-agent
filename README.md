...
Create a Render Blueprint from this repository, then enter `AICORE_BASE_URL`, `AICORE_AUTH_URL`, `AICORE_CLIENT_ID`, and `AICORE_CLIENT_SECRET` as environment variables in the Render dashboard. The blueprint selects deployment `dbb287aa19d03ae3` and model `gpt-4.1-nano`; override `AICORE_DEPLOYMENT_ID` and `AICORE_MODEL_NAME` together when selecting another deployment. Set `AICORE_RESOURCE_GROUP` to `default` unless your deployment uses another group. The included disk mounts at `/var/data`, so bookmark data persists across deploys and restarts. After deployment, copy the service URL into the extension's Backend URL field.
